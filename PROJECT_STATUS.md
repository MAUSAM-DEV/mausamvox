...
## ⚠️ START HERE NEXT SESSION — one manual step + acceptance tests (2026-07-05, second session)

**The 2026-07-05 acceptance pass PASSED** (engine quality ✅ compute 33.6s, Saved Tracks ✅, Performance Mode ✅). Three follow-up fixes + a favicon shipped the same day and now need their own pass:

0. **REQUIRED FIRST — apply migration `20260705000000` (instrumental_path column).** I can't run it (needs your Supabase access token). From the repo root:
   ```
   SUPABASE_ACCESS_TOKEN=<your token> node scripts/migrate.mjs
   ```
   Until it's applied, swaps still save fine (persist retries without the column) but no instrumental is stored and `/swaps/[id]` won't offer "Music only".
1. **Warm-ping fixes (`3629538`)** — one fresh full swap, then check Vercel logs: `[stem-split] warm-ping fired` should now appear **twice** (POST + stems-landed), `[rvc-warm] warm-ping fired` once when the result screen appears, and `[voice-convert] TIMING` queue should be ≈0 this time (last pass: 146.5s cold because the pool re-chills in <7 min). A regenerate a few minutes after reaching the result screen should also hit a warm pool now.
2. **Music-only Perform Live (`ea78cbd`)** — after the migration + one new swap: `/swaps/[id]` shows BOTH "🔊 Perform live" and "🎼 Perform live · music only"; music-only plays the instrumental (no vocal at all — clone or original); old swaps show only the full-track button + the honest fine-print. Delete a new swap → both files gone from storage (log line lists two paths).
3. **Headroom limiter (`923e9e3`)** — crank Warmth to max on a loud song: saved file should not crackle/distort at peaks (console shows a `[mixStems] peak … scaled` line when it engages).
4. **Favicon (`4187236`)** — browser tab shows the waveform mark; on the phone, "Add to Home Screen" shows the mark on the dark background.
