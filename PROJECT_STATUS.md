...
| Stem Studio (standalone page) | ✅ Working | Real `/stem-studio` page (`041ee22`), replacing the "coming soon" placeholder: upload MP3/WAV/M4A (75 MB) → Demucs 4-stem split → play + download each stem (Vocals/Bass/Drums/Other). Reuses existing machinery only (upload presign, `/api/stem-split`, `/api/stems/download`, shared AudioPlayer/VSidebar/VToast) — no new backend/DB. 50 credits on success (same `stem_split` price as the swap flow), free plan allowed, admin bypass. Auth-protected via middleware like voice-swap/voice-lab; live links in dashboard card + both sidebars (VSidebar gained `activeTool` prop). Deferred: instrumental/zip download, BPM/key detection (landing promises it), per-stem editing |
